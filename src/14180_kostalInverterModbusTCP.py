...
###################################################################################################!!!##
        ...
        self.internalRegisters = {
            ...
            self.INTERNAL_ENERGY_SCALEFACTOR: {'type': 'i16', 'regDec': 579, 'lastVal': 0, 'options': 0, 'calc': None, 'name': 'Internal:Energy scalefactor'},
            self.INTERNAL_POWER_SCALEFACTOR: {'type': 'i16', 'regDec': 576, 'lastVal': 0, 'options': 0, 'calc': None, 'name': 'Internal:Power scalefactor'}
        }
...
#############
    ...
    def read_power_values(self, client, unit_id):
        ...
        inverter_state = self.convert_to_inverter_status_string(self.registers[self.PIN_O_INVERTER_STATE_INT]['lastVal'])
    ...
    def read_i16_1(self, client, unit_id, reg_addr):
        result = client.read_holding_registers(reg_addr, 1, unit=unit_id)
        if not result.isError():
            return BinaryPayloadDecoder.fromRegisters(result.registers, byteorder=Endian.Big,
    ...
    def read16float_power_scaled(self, client, unit_id, reg_addr):
        result = client.read_holding_registers(reg_addr, 1, unit=unit_id)
        if not result.isError():
            int16_val = BinaryPayloadDecoder.fromRegisters(result.registers, byteorder=Endian.Big,
                                                      wordorder=self.word_order()).decode_16bit_int()
            return int16_val * 10 ** self.internalRegisters[self.INTERNAL_POWER_SCALEFACTOR]['lastVal']
        else:
            return -1
